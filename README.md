...
A set of data-specific functions to access the core ICCAT statistical datasets and extract core datasets in the format used for the SCRS and subsidiary bodies.

This library is meant for internal / development usage, as it provides access to the ICCAT databases (which are private) and therefore cannot be publicly disseminated as it is. 

Projects requiring access to ICCAT data (and meant for general audiences) shall include a snapshot of the data themselves (e.g., a subset of the `T1NC` dataset) as generated by **internal** ICCAT users through these libreries. 
 
## Artifacts that can be generated using the dataset-specific functions provided by the library

### T1NC - Task1 nominal catches
+ R [data table](https://cran.r-project.org/web/packages/data.table/data.table.pdf) / Excel file containing filtered Task1 **nominal catch** data (`T1NC`) for one ore more species

The format of the produced artifacts has the same exact structure and column names encountered in the ICCAT [Task1 Excel](https://iccat.int/Data/t1nc_20240131.7z) file 

### T2CE - Task2 catch and effort
1) R data table / Excel file containing filtered Task2 **catch and effort** data (`T2CE`) for one ore more species (or for a species family)
2) R data table / Excel file containing filtered Task2 **catch and effort data by operation mode** (`T2CE-ETRO`) limited to tropical fisheries only

>  The species family (when provided) should be one of those currently defined in column `FamilyTx` of the `dbSTAT.dbo.Species` table, i.e. those that can be identified through the following query:
```
SELECT
 DISTINCT FamilyTx
FROM
 dbSTAT.dbo.Species
WHERE
 FamilyTx IS NOT NULL
ORDER BY
 1 ASC
```

The format of the produced artifacts has the same exact structure and column names encountered in the following ICCAT files:

1) ICCAT [Task2 catch/effort detailed catalogue](https://iccat.int/Data/t2ce_detailedCatalogue.xlsx)
2) ICCAT [Task2 catch/effort by operation mode](https://iccat.int/Data/t2ce_PS91-22_bySchool.7z)

**Important notes**
> The provided functions only extract T2CE catch-and-effort records **with an associated species code**, i.e., they **cannot** be used to extract **T2CE records for strata with efforts only (and no catch)** 

### T2SZ and T2CS - Task2 observed size and estimated catch at size
1) R data table / Excel file containing filtered Task2 **observed size frequencies** (`T2SZ`) for one ore more species
2) R data table / Excel file containing filtered Task2 **estimated catch-at-size frequencies** (`T2CS`) for one ore more of the six species for which these are estimated
3) R data table / Excel file containing a combination of filtered Task2 **observed size frequencies** and **estimated catch-at-size frequencies** for one or more species

The format of the produced artifacts has the same exact structure and column names encountered in the following ICCAT files:

1) Species-specific [Task2 size-frequency (observed)](https://iccat.int/Data/t2size/t2sz/t2sz_ALB1950-22.7z)
2) Species-specific [Task2 catch-at-size (estimated)](https://iccat.int/Data/t2size/t2cs/t2cs_ALB1950-22.7z)
3) [Task2 size - detailed catalogue](https://iccat.int/Data/t2size/T2SZ-CS_detailedCatalogue.xlsx)
   
### SCRS catalogues
+ R data table representing filtered standard SCRS catalogues on statistics (Task 1 and Task 2) by species, stock, major fishery, and year, with flag / gear combinations ranked by order of importance.

The format of the produced artifact follows has exactly the same structure and column names encountered in each tab of the SCRS catalogues as disseminated by ICCAT for their [major species](https://iccat.int/Data/Catalogues/SCRScatalogues_MainSpecies_1993-22.xlsx)(10 tuna and tuna-like species and 3 shark species) as well as for [small tuna species](https://iccat.int/Data/Catalogues/SCRScatalogues_SmallTuna_1993-22.xlsx).

### CATDIS data (Task2 catch data raised to total landings by 5x5 degrees squares, quarter, and gear) 
+ R data table / Excel file containing filtered CATDIS data for one or more species

The format of the produced artifacts has exactly the same structure and column names encountered in the ICCAT [CATDIS (all)](https://iccat.int/Data/Catdis/cdis5022_all.7z) and [CATDIS (by species)](https://iccat.int/Data/Catdis/cdis5022_bySpecies.7z) files.

### Pre-filled statistical forms
1) `ST01` form (Excel file) containing filtered Task1 fishing craft statistics (both `ST01A` and `ST01B`, when available) for a given ICCAT CPC and time period (year from / to)
2) `ST02` form (Excel file) containing filtered Task1 nominal catch statistics for a given ICCAT CPC and time period (year from / to)
3) `ST03` form (Excel file) containing filtered Task2 catch and effort statistics for a given ICCAT CPC, time period (year from / to), and data source
4) `ST04` form (Excel file) containing filtered Task2 observed size frequency statistics for a given ICCAT CPC, time period (year from / to), species, product type, sampling location, sampling unit, frequency type, raised flag, class limit, and size interval
5) `ST05` form (Excel file) containing filtered Task2 catch and effort statistics for a given ICCAT CPC, time period (year from / to), species, frequency type, class limit, and size interval

The format of the produced artifacts has the same extact structure of the latest ICCAT (empty) ST forms 01-05 as downloadable from:

1) [ST01-T1FC](https://iccat.int/Forms/ST01-T1FC.xlsx) (Req. `S:GEN02`) - Task1 fleet characteristics (`T1FC`)
2) [ST02-T1NC](https://iccat.int/Forms/ST02-T1NC.xlsx) (Req. `S:GEN03`) - Task1 nominal catch estimations (`T1NC`)
3) [ST03-T2CE](https://iccat.int/Forms/ST03-T2CE.xlsx) (Req. `S:GEN04`) - Task2 catch and effort (`T2CE`)
4) [ST04-T2SZ](https://iccat.int/Forms/ST04-T2SZ.xlsx) (Req. `S:GEN05`) - Task2 size samples (`T2SZ`)
5) [ST05-T2CS](https://iccat.int/Forms/ST05-T2CS.xlsx) (Req. `S:GEN06`) - Task2 catch-at-size estimations (`T2CS`) 

respectively.

> As the actual content of the `ST0x` forms can in principle be extracted from one of the various ICCAT db (`production`, `preliminary`, `historical`, etc.), it is up to the **internal** ICCAT user to guarantee that the correct source is selected prior to disseminating the results.
> Generally speaking, the `ST0x` forms should be filled with data stored in the `production` databases (either `T1` or `dbSTAT`) if their purpose is to be disseminated to CPC users (either directly, or through future integration with IOMS).
...
### Installation
```
install.packages(c("openxlsx", "openxlsx2", "stringr"))
```

## Internal dependencies <a name="internal_deps"></a>
+ [iccat.pub.base](https://github.com/stats-ICCAT/iccat.pub.base) (*indirect* dependency)
+ [iccat.pub.data](https://github.com/stats-ICCAT/iccat.pub.data)
+ [iccat.dev.base](https://github.com/stats-ICCAT/iccat.dev.base)

### Installation (straight from GitHub)
```
library(devtools)

install_github("stats-ICCAT/iccat.pub.base")
install_github("stats-ICCAT/iccat.pub.data")
install_github("stats-ICCAT/iccat.dev.base")
```
